...
@router.message(
    Command("cancel"),
    StateFilter(Upload.waiting_for_title, Upload.waiting_for_file, Broadcast.waiting_for_message),
)
...
async def process_title(message: types.Message, state: FSMContext) -> None:
    ...
    expected_type = "video" if category == "video" else "audio"

    await state.update_data(title=title)
    await state.set_state(Upload.waiting_for_file)
    await message.answer(f"Now send the {expected_type} file.")
...
async def process_file(message: types.Message, state: FSMContext) -> None:
    ...
    content_id = add_content(title, category, file_id)
    await state.clear()
    ...
    if health.get("ok"):
        ...
        lines.extend(
            [
                ...
                f"Collections -> users: {collections.get('users', 0)}, content: {collections.get('content', 0)}, counters: {collections.get('counters', 0)}",
